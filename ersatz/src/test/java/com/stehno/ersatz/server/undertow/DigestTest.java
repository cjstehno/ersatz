...
class DigestTest {
    ...
    private ErsatzServer ersatzServer = new ErsatzServer(c -> {
        c.authentication(AuthenticationConfig::digest);
        c.expects().GET("/secrets").responds().body("Something secure");
    });

    @Test @DisplayName("DIGEST no-auth")
    void digestNoAuth() throws IOException {
    ...
    @Test @DisplayName("DIGEST auth")
    void digestAuth() throws IOException {
