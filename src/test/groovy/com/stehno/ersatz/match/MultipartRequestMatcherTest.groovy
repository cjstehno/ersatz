...
class MultipartRequestMatcherTest {
    ...
    @Test @DisplayName('configured with closure')
    ...
    @Test @DisplayName('configured with consumer')
    void configuredWithConsumer() {
        assertTrue(
    ...
    void partFieldMatcher(final String name, Matcher m, final boolean result) {
    ...
    void partFieldContentMatcher(String name, Matcher contentMatcher, Matcher valueMatcher, boolean result) {
    ...
    private static Stream<Arguments> partEverythingProvider() {
        Stream.of(
            arguments(newMatcher().part('alpha', nullValue(), startsWith('text/plain'), equalTo('one')), true),
            arguments(newMatcher().part('alpha', nullValue(), startsWith('text/plain'), equalTo('two')), false),
