...
class ErsatzServer implements Closeable {
    ...
    String httpUrl(final String path) {
        underlyingServer.getHttpUrl(path)
    }
    ...
    String httpsUrl(final String path) {
        underlyingServer.getHttpsUrl(path)
    }
    ...
    boolean verify(final long timeout = 1, final TimeUnit unit = SECONDS) {
        serverConfig.verify(timeout, unit)
    }
}
