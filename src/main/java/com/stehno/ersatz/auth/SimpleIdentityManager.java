...
public class SimpleIdentityManager implements IdentityManager {
    ...
    private static final Logger log = LoggerFactory.getLogger(SimpleIdentityManager.class);
    ...
    public Account verify(final String id, final Credential credential) {
        if (credential instanceof PasswordCredential) {
            if (username == id && password == valueOf(((PasswordCredential) credential).getPassword())) {
            ...
            try {
                ...
                final byte[] bytes = digestCredential.getAlgorithm().getMessageDigest().digest((username + ":" + digestCredential.getRealm() + ":" + password).getBytes());
                if (digestCredential.verifyHA1(convertToHexBytes(bytes))) {
                    return new SimpleAccount(id);
                }

            } catch (NoSuchAlgorithmException e) {
                log.error("Digest credential algorithm is unavailable: {}", e.getMessage());
                return null;
            }
        }
