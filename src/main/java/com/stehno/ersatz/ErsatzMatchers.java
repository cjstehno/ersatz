...
public class ErsatzMatchers {
     ...
     */
    public static <T> Matcher<Iterable<T>> collectionContains(final T value) {
        return new CollectionContainsValueMatcher<T>(value);
    }
    ...
    private static class CollectionContainsMatchMatcher<T> extends BaseMatcher<Iterable<T>> {
        ...
        private CollectionContainsMatchMatcher(final Matcher<T> matcher) {
            this.matcher = matcher;
        }

        @Override
        public boolean matches(final Object item) {
            return item != null && ((Collection<T>) item).stream().anyMatch(matcher::matches);
        }
    ...
    private static class CollectionContainsValueMatcher<T> extends BaseMatcher<Iterable<T>> {
        ...
        private CollectionContainsValueMatcher(final T value) {
            this.value = value;
        }

        @Override
        public boolean matches(final Object item) {
            return item != null && ((Collection<T>) item).contains(value);
        }
    ...
    private static class ByteArrayMatcher extends BaseMatcher<byte[]> {
        ...
        private ByteArrayMatcher(final byte[] array) {
            this.array = array;
        }
