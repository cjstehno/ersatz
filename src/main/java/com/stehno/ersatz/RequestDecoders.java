...
public class RequestDecoders {
    ...
    public void register(final String contentType, final BiFunction<byte[], DecodingContext, Object> decoder) {
        decoders.stream()
            .filter(c -> c.getMimeType().toString().equals(contentType))
            .findFirst()
    ...
    public BiFunction<byte[], DecodingContext, Object> findDecoder(final String contentType) {
        ...
        final List<DecoderMapping> found = decoders.stream()
            .filter(c -> c.getMimeType().match(mimeType))
        ...
        if (found.isEmpty()) {
            ...
            return found.stream()
                .filter(f -> f.getMimeType().toString().equals(contentType))
                .findFirst()
                .map(DecoderMapping::getDecoder)
                .orElse(null);

        } else {
            return found.get(0).getDecoder();
        }
    }

    private static MimeType createMimeType(final String type) {
        try {
    ...
    private static class DecoderMapping {
        ...
        DecoderMapping(final MimeType mimeType, final BiFunction<byte[], DecodingContext, Object> decoder) {
        ...
        public BiFunction<byte[], DecodingContext, Object> getDecoder() {
            return decoder;
        }
    }
}