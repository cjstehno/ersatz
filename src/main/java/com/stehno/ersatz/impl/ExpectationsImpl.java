...
public class ExpectationsImpl implements Expectations {
    ...
    public RequestWithContent POST(Matcher<String> matcher, Consumer<RequestWithContent> consumer) {
        final RequestWithContent request = new ErsatzRequestWithContent(HttpMethod.POST, matcher, globalDecoders, globalEncoders);
    ...
    @Override
    public RequestWithContent PUT(Matcher<String> matcher, Consumer<RequestWithContent> consumer) {
        final ErsatzRequestWithContent request = new ErsatzRequestWithContent(HttpMethod.PUT, matcher, globalDecoders, globalEncoders);
        consumer.accept(request);
        requests.add(request);
        return request;
    }
    ...
    @Override
    public RequestWithContent PATCH(Matcher<String> matcher, Consumer<RequestWithContent> consumer) {
        final ErsatzRequestWithContent request = new ErsatzRequestWithContent(HttpMethod.PATCH, matcher, globalDecoders, globalEncoders);
        consumer.accept(request);
        requests.add(request);
        return request;
    }
    ...
    private static void verification(final boolean check, final String message) {
        if (!check) {
    ...
    public boolean verify(final long timeout, final TimeUnit unit) {
        for (final Request r : requests) {
            verification(((ErsatzRequest) r).verify(timeout, unit), "Expectations for " + r + " were not met.");
        }

        for (final WebSocketExpectations ws : webSockets.values()) {
            verification(((WebSocketExpectationsImpl) ws).verify(timeout, unit), "WebSocket expectations for " + ws + " were not met.");
        }
    ...
    private Request expect(final Request request, final Consumer<? super Request> consumer) {
        consumer.accept(request);
        requests.add(request);
        return request;
    }
