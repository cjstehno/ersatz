...
public class WebSocketExpectationsImpl implements WebSocketExpectations {
    ...
    public ReceivedMessage receive(Consumer<ReceivedMessage> config) {
        ReceivedMessageImpl message = new ReceivedMessageImpl(null, null);
    ...
    public SentMessage send(Consumer<SentMessage> config) {
        SentMessageImpl message = new SentMessageImpl(null, null);
        config.accept(message);
        sentMessages.add(message);
        return message;
    }
    ...
    public ReceivedMessageImpl findMatch(final Object message) {
        return receivedMessages.stream().filter(m -> m.matches(message)).findFirst().orElse(null);
    }

    public boolean verify(final long timeout, final TimeUnit unit) {
        try {
            return connectionLatch.await(timeout, unit) && receivedMessages.stream().allMatch(m -> m.marked(timeout, unit));
