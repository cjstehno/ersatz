...
public class ReceivedMessageImpl implements ReceivedMessage {
    ...
    public ReceivedMessageImpl(final Object payload, final WsMessageType messageType) {
    ...
    public boolean matches(final Object message) {
        if (message instanceof BufferedBinaryMessage) {
            return matches((BufferedBinaryMessage) message);
        } else if (message instanceof BufferedTextMessage) {
            return matches((BufferedTextMessage) message);
        } else {
            return false;
        }
    }

    public boolean matches(BufferedBinaryMessage message) {
        byte[] incomingBytes = new byte[0];

        for (final ByteBuffer b : message.getData().getResource()) {
            final byte[] data = new byte[b.remaining()];
            b.get(data);
            // FIXME: add data array to incomignBytes arreay - System.arraycopy(data, 0, incomingBytes, incomingBytes.length, data.length);
        }

        return messageType == BINARY && incomingBytes.equals(payload);
    }
