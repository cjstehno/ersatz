...
public class RequestMatcher extends BaseMatcher<ClientRequest> {
    ...
    private final Function<ClientRequest, ?> getter;
    ...
    public RequestMatcher(final Matcher<?> matcher, final Function<ClientRequest, ?> getter, final String description) {
    ...
    public static RequestMatcher protocol(final Matcher<String> m) {
        return new RequestMatcher(m, (Function<ClientRequest, String>) ClientRequest::getProtocol, "Protocol matches ");
    }
    ...
    public static RequestMatcher method(final Matcher<HttpMethod> m) {
        return new RequestMatcher(m, (Function<ClientRequest, HttpMethod>) ClientRequest::getMethod, "HTTP method matches ");
    }
    ...
    public static RequestMatcher path(final Matcher<String> m) {
        return new RequestMatcher(m, (Function<ClientRequest, String>) ClientRequest::getPath, "Path matches ");
    }
    ...
    public static RequestMatcher header(final String name, final Matcher<Iterable<? super String>> m) {
        return new RequestMatcher(
            m,
            (Function<ClientRequest, Iterable<String>>) cr -> cr.getHeaders().get(name),
            "Header " + name + " matches "
        );
    }
     ...
     */
    public static RequestMatcher param(final String name, final Matcher<Iterable<? extends String>> m) {
        return new RequestMatcher(
            m,
            (Function<ClientRequest, Iterable<String>>) cr -> {
                try {
                    return parseQueryString(new String(cr.getBody(), UTF_8), UTF_8).get(name);
    ...
    public static RequestMatcher cookie(final String name, final Matcher<Cookie> m) {
        return new RequestMatcher(
            m,
            (Function<ClientRequest, Cookie>) cr -> {
                final io.undertow.server.handlers.Cookie undertowCookie = cr.getCookies().get(name);
    ...
    public static RequestMatcher cookies(final Matcher<Map<String, Cookie>> matcher) {
        return new RequestMatcher(
            matcher,
            (Function<ClientRequest, Map<String, Cookie>>) cr -> {
                final Map<String, Cookie> cookies = new HashMap<>();
                cr.getCookies().forEach((name, cookie) -> {
                    cookies.put(name, bake(cookie));
                });
                return cookies;
            },
            "Cookies match "
    ...
    public static RequestMatcher body(final DecoderChain decoderChain, final String contentType, final Matcher<Object> m) {
        return new RequestMatcher(m,
            cr -> decoderChain.resolve(contentType).apply(
                cr.getBody(),
                new DecodingContext(cr.getContentLength(), cr.getContentType(), cr.getCharacterEncoding(), decoderChain)
            ),
