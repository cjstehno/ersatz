...
instance of the `MultipartContent` class and configuring its parts and encoders.
...
content object. These configurations are made on the `MultipartContent` object via DSL or functional interface.
...
ersatzServer.expectations {
    get('/data') {
        responds().content(multipart {
            ...
            encoder IMAGE_JPG, File, { o -> ((File)o).bytes.encodeBase64(true) }

            // a field part
            field 'comments', 'This is a cool image.'

            // a file part
            part 'image', 'test-image.jpg', IMAGE_JPG, new File('/test-image.jpg')
        })
    }
}
----
