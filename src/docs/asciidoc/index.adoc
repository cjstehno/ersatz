...
TIP: You can set the server to start automatically after expectations are applied by adding the `autoStart()` configuration to the global configuration
closure.
...
calling the `ErsatzServer::stop()` method. This is an important step, as odd test failures have been noticed during multi-test runs if the server is
not properly stopped. In Spock you can create the `ErsatzServer` with the `@AutoCleanup` annotation to aid in proper management:
...
method is called. This can simplify the code by removing explicit calls to the `start()` method. There are two configuration methods for the
auto-start feature. The first one, is simply a means of enabling auto-start:
...
The explicit `start()` call is required since there are no expecations specified (auto-start wont fire). The `HttpUrlConnection` is used to make the
...
* [Consume and test a third-party REST API](http://guides.grails.org/grails-mock-http-server/guide/index.html) _(Sergio del Amo)_ - Use Ersatz, a "mock" HTTP library, for testing code dealing with HTTP requests
