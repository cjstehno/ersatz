...
* Requires Java 15+
* Removed support for legacy JUnit (< v5).
* Added new option to enable response content rendering (disabled by default).
...
* Removed built-in support for JSON encoding/decoding so that no specific implementation library was required. Documentation has been provided to show implementations using some common JSON support libraries.
...
* Added more pre-defined `ContentType` constants.
* Added more encoders and decoders for common scenarios.
* Updated the dependencies and fixed some isolation issues.

=== Migrating to 3.0

* If you use the proxy or web sockets testing support, there is no upgrade path. Please create an [Issue](https://github.com/cjstehno/ersatz/issues) to show that you are interested in one or both of these features.
* If develop with Groovy, you will need to change your dependency references from using the `ersatz` library to use the new `ersatz-groovy` library. The same change applies if you are using the `safe` version of the library.
* If you are using the legacy JUnit support helper, you will either need to implement the support class yourself (optionally using the source from the 2.x codebase), or you can submit an [Issue](https://github.com/cjstehno/ersatz/issues) and if there is enough interest a solution could be added back into the library.
...
As an example, a JSON decoder based on the Groovy JSON parser could look like the following code:
...
As of 3.0.0 the built-in JSON Decoder was removed (to avoid dependency lock-in). Below are provided some simple/common implementations of them for use in your code:

*Groovy.* A decoder (parser) implemented with the Groovy `JsonSlurper` could be implemented as:
